...
#include <stdio.h>   //stdio是标准io函数，比如printf和scanf函数
...
{
    HANDLE handle = GetStdHandle(STD_OUTPUT_HANDLE);//标准输出句柄，就像初始化显示器，为坐标做准备  
    COORD pos;    //COORD用来表示一个字符在控制台显示屏上的坐标  
    pos.X = x;
    pos.Y = y;
    SetConsoleCursorPosition(handle,pos);  //定位 
}
